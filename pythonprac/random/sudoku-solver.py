...
#This is where i got stuck. this was due to currentboard not resetting after every recursive call.
import time
...
def solveSudoku(board,m):
    ...
    for x in range(1,10):
        currentBoard[boardX][boardY]=str(x)
        if m:
            print(currentBoard)
        ...
        if isValidSudoku(currentBoard):
            ...
            if solveSudoku(currentBoard,False)==True:
